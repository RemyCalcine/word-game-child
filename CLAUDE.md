...
- `src/screens/` — un composant par écran (`StartScreen`, `LearnScreen`, `SyllablesScreen`, `WriteScreen`, `WinScreen`, `RecapScreen`, `WordEditor`, plus `screens/nether/` pour le mode Nether). `WriteScreen` est réutilisé pour l'écriture normale et pour l'épreuve Nether via une prop (le mode Nether ajoute les boutons « Nouvelle tentative » / « Passer »). `RecapScreen` est la page de récap commune (fin d'overworld + sortie de Nether) ; elle porte les boutons **Rejouer** / **Quitter la partie** (retour à l'accueil). `WordEditor` est l'éditeur de mots (modal ⚙️) : accessible **uniquement depuis l'accueil**, présenté en **tableau** (1 ligne = 1 mot), réordonnancement par glisser-déposer (☰) et flèches ▲▼, aperçu syllabes en direct, champ indice + case 🔥 Nether par mot, prénom de l'enfant. Bouton **⏸ mettre de côté** : le mot bascule dans une section **« Mots de côté »** (désactivé : `actif:false`) sans être supprimé, d'où on peut le **▶ Réactiver** (revient en bas des actifs) ; un mot de côté est exclu du jeu **et** du Nether. Bouton **↺ Reset** : recharge la liste par défaut de `words.js` (avec confirmation, effectif au **Valider** seulement). Il applique à la validation.
...
- `App.jsx` tient l'état React (écran courant, index du mot, score, config) et dérive `motsListe` via `prepareList()` sur les `entrees` **actives** de la config (`entrees.filter(e => e.actif !== false)`) ; l'éditeur ⚙️ met à jour ces entrées et `saveConfig()` les persiste. Chaque entrée porte un champ `actif` (défaut `true`, rétrocompatible) : un mot mis de côté (`actif:false`) est exclu du jeu et du Nether (`motsNether` dérive de `motsListe`), tout en restant dans la config.
