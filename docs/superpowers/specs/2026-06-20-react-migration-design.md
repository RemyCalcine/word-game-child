...
4. **NetherEndScreen** — récapitulatif (« Nether vaincu ! », mots réussis,
   diamants gagnés dans le Nether = `netherWordsOk * 15`, dérivés à
   l'affichage, pas de champ d'état séparé), bouton retour à l'accueil.
...
10. Mettre à jour `CLAUDE.md` : stack (Vite + React), commande de lancement
    ...
    nouveau champ `nether` dans `words.js`, et **supprimer/réécrire** la
    phrase « aucun `fetch`, donc `file://` fonctionne » (devenue fausse,
    cf. Risques).
